          ...
          <div className="grid auto-rows-min gap-4 md:grid-cols-3">
            ...
            <div className="bg-muted/50 aspect-video rounded-xl">
              <div className="flex">
                ...
                <div className="items-center justify-end mt-4 flex ml-auto mr-6 gap-4">
                  <span className="w-2.5 h-2.5 rounded-full bg-orange-400 animate-pulse shadow-glow"></span>
                  <h1 className="text-xl font-semibold -mt-1">
              ...
              {(repairItems?.slice(0, 3) ?? []).length > 0 ? (
                repairItems.slice(0, 3).map(item => (
                  <div className="flex gap-6 ml-6 mt-6 border-b-1 pb-2 max-w-[90%]" key={item.id}>
                    <h1 className="text-zinc-300">{item.user.email}</h1>
                    <h1 className="text-orange-400 font-regular ml-auto mr-2 font-semibold">
                      ITEM: <span className="underline font-bold">{item.item.serialCode}</span>
                    </h1>
                  </div>
                ))
              ) : (
                <div className="text-center mt-24 text-zinc-400 font-medium">No items for repair found.</div>
              )}
            </div>
            <div className="bg-muted/50 aspect-video rounded-xl">
              <div className="flex">
          ...
          <div className="flex-1 rounded-xl bg-muted/50 p-4 md:h-auto max-h-[600px]">
            <ScrollArea className="h-full w-full">
              <div className="flex items-center justify-between mb-6">
                <h1 className="text-xl font-semibold">Latest requests</h1>
                  ...
                  <DropdownMenu key={req.id}>
                    <div className="flex items-center justify-between border-1 text-white px-4 py-3 rounded-md">
                      ...
                      <div className="flex items-center gap-6 text-sm text-zinc-300">
                        ...
                        <div>
                          <span className="font-semibold text-white">FOR:</span> {req.item.serialCode}
                        </div>
